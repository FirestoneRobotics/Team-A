...
task autonomous()
{
	...
	{
		motor[rightF] = motor[rightB] = motor[leftF] = motor[leftB] = 127;
...
while(SensorValue(driveSensor) > -2999)
{
	motor[rightF] = motor[rightB] = motor[leftF] = motor[leftB] = -127;
}
  stopDrivingClearSensor();
...
task usercontrol()
{
	while (true)
	{
		...
		if(vexRT[Btn5U] == 1)
		{
			motor[armMotorL] = motor[armMotorR] = 127;

		}
		else if(vexRT [Btn5D] == 1)
		{
			motor[armMotorL] = motor[armMotorR] = -20;
		}
		else
		{
			motor[armMotorL] = motor[armMotorR] = 20;
